...
{	int j, h = now._nr_pr;
	...
	case 6:	/* claim select */
		...
		if (whichclaim == -1 && claimname == NULL)
			printf("0: Claim Safe (3), from state 5\n");
		if (whichclaim != -1)
	...
	case 1:	/* Elf */
		...
		((P1 *)pptr(h))->id = par0;
		/* locals: */
		((P1 *)pptr(h))->_5_toy_to_make = 0;
		((P1 *)pptr(h))->_5_toys_made = 0;
#ifdef VAR_RANGES
		logval("Elf:id", ((P1 *)pptr(h))->id);
		logval("Elf:toys_made", ((P1 *)pptr(h))->_5_toys_made);
		logval("Elf:toy_to_make", ((P1 *)pptr(h))->_5_toy_to_make);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Santa */
		...
		/* locals: */
		((P0 *)pptr(h))->_4_elf = 0;
		((P0 *)pptr(h))->_4_total_assigned = 0;
		((P0 *)pptr(h))->_4_nstops = 0;
		((P0 *)pptr(h))->_4_next_toy = 0;
#ifdef VAR_RANGES
		logval("Santa:elf", ((P0 *)pptr(h))->_4_elf);
		logval("Santa:total_assigned", ((P0 *)pptr(h))->_4_total_assigned);
		logval("Santa:nstops", ((P0 *)pptr(h))->_4_nstops);
		logval("Santa:next_toy", ((P0 *)pptr(h))->_4_next_toy);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
		now.dolls = 0;
		now.trains = 0;
		now.to_santa = addqueue(1, 0);
		now.to_elf[0] = addqueue(2, 1);
		now.to_elf[1] = addqueue(3, 1);
		now.to_elf[2] = addqueue(4, 1);
#ifdef VAR_RANGES
		logval("dolls", now.dolls);
		logval("trains", now.trains);
#endif
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.to_santa) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.to_santa) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.to_santa) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.to_santa) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.to_elf[0]) )
			 ||     q_full((int) ( now.to_elf[1]) )
			 ||     q_full((int) ( now.to_elf[2]) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.to_elf[0]) )
			 ||    !q_full((int) ( now.to_elf[1]) )
			 ||    !q_full((int) ( now.to_elf[2]) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.to_elf[0]) )
			 ||     !q_len((int) ( now.to_elf[1]) )
			 ||     !q_len((int) ( now.to_elf[2]) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.to_elf[0]) )
			 ||      q_len((int) ( now.to_elf[1]) )
			 ||      q_len((int) ( now.to_elf[2]) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	byte   dolls:	%d\n", now.dolls);
	printf("	byte   trains:	%d\n", now.trains);
	printf("	chan to_santa (=%d):	len %d:\t", now.to_santa, q_len(now.to_santa));
	c_chandump(now.to_santa);
	printf("	chan to_elf[0] (=%d):	len %d:\t", now.to_elf[0], q_len(now.to_elf[0]));
	c_chandump(now.to_elf[0]);
	printf("	chan to_elf[1] (=%d):	len %d:\t", now.to_elf[1], q_len(now.to_elf[1]));
	c_chandump(now.to_elf[1]);
	printf("	chan to_elf[2] (=%d):	len %d:\t", now.to_elf[2], q_len(now.to_elf[2]));
	c_chandump(now.to_elf[2]);
}
void
...
{	/* int i; */
	...
	printf("	byte   i:	%d\n", ((P2 *)pptr(pid))->_6_i);
		break;
	case 1:
		printf("local vars proc %d (Elf):\n", pid);
	printf("	byte   id:	%d\n", ((P1 *)pptr(pid))->id);
	printf("	byte   toys_made:	%d\n", ((P1 *)pptr(pid))->_5_toys_made);
		break;
	case 0:
		printf("local vars proc %d (Santa):\n", pid);
	printf("	byte   elf:	%d\n", ((P0 *)pptr(pid))->_4_elf);
	printf("	byte   total_assigned:	%d\n", ((P0 *)pptr(pid))->_4_total_assigned);
	printf("	byte   nstops:	%d\n", ((P0 *)pptr(pid))->_4_nstops);
		break;
	}
}
void
printm(int x)
{
	...
	case 2: Printf("TRAIN"); break;
	case 3: Printf("DOLL"); break;
	default: Printf("%d", x);
	}
}
void
