...
/*** From source: shift-alt-esc2.pml ***/
...
{	int j, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 9:	/* claim select */
		spin_c_typ[0] = 7; /* Liveness2 */
		((P9 *)pptr(h))->c_cur[0] = 5;
		reached7[5]=1;
		spin_c_typ[1] = 6; /* Liveness */
		((P9 *)pptr(h))->c_cur[1] = 5;
		reached6[5]=1;
		spin_c_typ[2] = 5; /* Safety2 */
		((P9 *)pptr(h))->c_cur[2] = 3;
		reached5[3]=1;
		spin_c_typ[3] = 4; /* Safety */
		((P9 *)pptr(h))->c_cur[3] = 7;
		reached4[7]=1;
		((P9 *)pptr(h))->_t = 4;
		((P9 *)pptr(h))->_p = 7;
		((P9 *)pptr(h))->_n = 3; /* Safety */
		src_claim = src_ln4;
		if (whichclaim == -1 && claimname == NULL)
			printf("0: Claim Safety (4), from state 7\n");
		if (whichclaim != -1)
	...
	case 2:	/* Server */
		...
		((P2 *)pptr(h))->id = par0;
		/* locals: */
		((P2 *)pptr(h))->_5_customer_order.customer = 0;
		((P2 *)pptr(h))->_5_customer_order.food = 0;
		((P2 *)pptr(h))->_5_customer_order.is_ordered = 0;
		((P2 *)pptr(h))->_5_customer_order.is_cooked = 0;
		((P2 *)pptr(h))->_5_customer_order.is_ready = 0;
		((P2 *)pptr(h))->_5_cID = 0;
#ifdef VAR_RANGES
		logval("Server:id", ((P2 *)pptr(h))->id);
		logval("Server:cID", ((P2 *)pptr(h))->_5_cID);
		logval("Server:_5_customer_order.customer", ((P2 *)pptr(h))->_5_customer_order.customer);
		logval("Server:_5_customer_order.food", ((P2 *)pptr(h))->_5_customer_order.food);
		logval("Server:_5_customer_order.is_ordered", ((P2 *)pptr(h))->_5_customer_order.is_ordered);
		logval("Server:_5_customer_order.is_cooked", ((P2 *)pptr(h))->_5_customer_order.is_cooked);
		logval("Server:_5_customer_order.is_ready", ((P2 *)pptr(h))->_5_customer_order.is_ready);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Cashier */
		...
		((P1 *)pptr(h))->id = par0;
		/* locals: */
		((P1 *)pptr(h))->_4_customer_order.customer = 0;
		((P1 *)pptr(h))->_4_customer_order.food = 0;
		((P1 *)pptr(h))->_4_customer_order.is_ordered = 0;
		((P1 *)pptr(h))->_4_customer_order.is_cooked = 0;
		((P1 *)pptr(h))->_4_customer_order.is_ready = 0;
		((P1 *)pptr(h))->_4_cID = 0;
#ifdef VAR_RANGES
		logval("Cashier:id", ((P1 *)pptr(h))->id);
		logval("Cashier:cID", ((P1 *)pptr(h))->_4_cID);
		logval("Cashier:_4_customer_order.customer", ((P1 *)pptr(h))->_4_customer_order.customer);
		logval("Cashier:_4_customer_order.food", ((P1 *)pptr(h))->_4_customer_order.food);
		logval("Cashier:_4_customer_order.is_ordered", ((P1 *)pptr(h))->_4_customer_order.is_ordered);
		logval("Cashier:_4_customer_order.is_cooked", ((P1 *)pptr(h))->_4_customer_order.is_cooked);
		logval("Cashier:_4_customer_order.is_ready", ((P1 *)pptr(h))->_4_customer_order.is_ready);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Customer */
		...
		/* params: */
		((P0 *)pptr(h))->id = par0;
		((P0 *)pptr(h))->favorite = par1;
		/* locals: */
#ifdef VAR_RANGES
		logval("Customer:id", ((P0 *)pptr(h))->id);
		logval("Customer:favorite", ((P0 *)pptr(h))->favorite);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
	{	int l_in;
		...
		{
			now.wants_order[l_in] = 0;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.places_order[l_in] = 0;
		}
	}
		now.server_busy = 0;
		now.customer_waiting = 0;
		now.cashier_ready = 0;
		now.server_ready = 0;
		now.ready_customer = 0;
		now.ready_order = 0;
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.server_got[l_in] = 0;
		}
	}
		now.orders[0].customer = 0;
		now.orders[0].food = 0;
		now.orders[0].is_ordered = 0;
		now.orders[0].is_cooked = 0;
		now.orders[0].is_ready = 0;
		now.orders[1].customer = 0;
		now.orders[1].food = 0;
		now.orders[1].is_ordered = 0;
		now.orders[1].is_cooked = 0;
		now.orders[1].is_ready = 0;
		now.orders[2].customer = 0;
		now.orders[2].food = 0;
		now.orders[2].is_ordered = 0;
		now.orders[2].is_cooked = 0;
		now.orders[2].is_ready = 0;
#ifdef VAR_RANGES
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("wants_order[l_in]", now.wants_order[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("places_order[l_in]", now.places_order[l_in]);
		}
	}
		logval("server_busy", now.server_busy);
		logval("customer_waiting", now.customer_waiting);
		logval("cashier_ready", now.cashier_ready);
		logval("server_ready", now.server_ready);
		logval("ready_customer", now.ready_customer);
		logval("ready_order", now.ready_order);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("server_got[l_in]", now.server_got[l_in]);
		}
	}
		logval("orders[0].customer", now.orders[0].customer);
		logval("orders[0].food", now.orders[0].food);
		logval("orders[0].is_ordered", now.orders[0].is_ordered);
		logval("orders[0].is_cooked", now.orders[0].is_cooked);
		logval("orders[0].is_ready", now.orders[0].is_ready);
		logval("orders[1].customer", now.orders[1].customer);
		logval("orders[1].food", now.orders[1].food);
		logval("orders[1].is_ordered", now.orders[1].is_ordered);
		logval("orders[1].is_cooked", now.orders[1].is_cooked);
		logval("orders[1].is_ready", now.orders[1].is_ready);
		logval("orders[2].customer", now.orders[2].customer);
		logval("orders[2].food", now.orders[2].food);
		logval("orders[2].is_ordered", now.orders[2].is_ordered);
		logval("orders[2].is_cooked", now.orders[2].is_cooked);
		logval("orders[2].is_ready", now.orders[2].is_ready);
#endif
...
{	/* int i; */
	...
	printf("	byte   customer:	%d\n", now.orders[ 0 ].customer);
	printf("	byte   is_ordered:	%d\n", now.orders[ 0 ].is_ordered);
	printf("	byte   is_cooked:	%d\n", now.orders[ 0 ].is_cooked);
	printf("	byte   is_ready:	%d\n", now.orders[ 0 ].is_ready);
	printf("	byte   customer:	%d\n", now.orders[ 1 ].customer);
	printf("	byte   is_ordered:	%d\n", now.orders[ 1 ].is_ordered);
	printf("	byte   is_cooked:	%d\n", now.orders[ 1 ].is_cooked);
	printf("	byte   is_ready:	%d\n", now.orders[ 1 ].is_ready);
	printf("	byte   customer:	%d\n", now.orders[ 2 ].customer);
	printf("	byte   is_ordered:	%d\n", now.orders[ 2 ].is_ordered);
	printf("	byte   is_cooked:	%d\n", now.orders[ 2 ].is_cooked);
	printf("	byte   is_ready:	%d\n", now.orders[ 2 ].is_ready);
	printf("	byte   customer_waiting:	%d\n", now.customer_waiting);
	printf("	byte   cashier_ready:	%d\n", now.cashier_ready);
	printf("	byte   server_ready:	%d\n", now.server_ready);
	printf("	byte   ready_customer:	%d\n", now.ready_customer);
	printf("	byte   ready_order:	%d\n", now.ready_order);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    wants_order[%d]:	%d\n", l_in, now.wants_order[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    places_order[%d]:	%d\n", l_in, now.places_order[l_in]);
		}
	}
	printf("	bit    server_busy:	%d\n", now.server_busy);
}
void
...
{	/* int i; */
	...
	printf("	byte   id:	%d\n", ((P2 *)pptr(pid))->id);
		printf("	(struct _5_customer_order)\n");
	printf("	byte   customer:	%d\n", ((P2 *)pptr(pid))->_5_customer_order.customer);
	printf("	byte   is_ordered:	%d\n", ((P2 *)pptr(pid))->_5_customer_order.is_ordered);
	printf("	byte   is_cooked:	%d\n", ((P2 *)pptr(pid))->_5_customer_order.is_cooked);
	printf("	byte   is_ready:	%d\n", ((P2 *)pptr(pid))->_5_customer_order.is_ready);
	printf("	byte   cID:	%d\n", ((P2 *)pptr(pid))->_5_cID);
		break;
	case 1:
		printf("local vars proc %d (Cashier):\n", pid);
	printf("	byte   id:	%d\n", ((P1 *)pptr(pid))->id);
		printf("	(struct _4_customer_order)\n");
	printf("	byte   customer:	%d\n", ((P1 *)pptr(pid))->_4_customer_order.customer);
	printf("	byte   is_ordered:	%d\n", ((P1 *)pptr(pid))->_4_customer_order.is_ordered);
	printf("	byte   is_cooked:	%d\n", ((P1 *)pptr(pid))->_4_customer_order.is_cooked);
	printf("	byte   is_ready:	%d\n", ((P1 *)pptr(pid))->_4_customer_order.is_ready);
	printf("	byte   cID:	%d\n", ((P1 *)pptr(pid))->_4_cID);
		break;
	case 0:
		printf("local vars proc %d (Customer):\n", pid);
	printf("	byte   id:	%d\n", ((P0 *)pptr(pid))->id);
		break;
	}
}
...
printm(int x)
{
	...
	case 2: Printf("SANDWICH"); break;
	case 3: Printf("CHILI"); break;
	default: Printf("%d", x);
	}
}
void
