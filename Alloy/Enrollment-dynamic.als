...
pred enroll[p : Person, c : Course, st : Step] {
	p not in c.enrolled.st
	#c.enrolled.st < c.limit
	c not in p.transcript
	c.prereqs in p.transcript

	let st' = next[st] {
		enrolled.st' = enrolled.st + c->p
	}
}

assert enroll_closed {
	all p : Person, c : Course, st : Step - last {
		Invariant[st] && enroll[p, c, st] => Invariant[next[st]]
	}
}
...
pred enroll_exists {
	some p : Person, c : Course, st : Step {
		Invariant[st]
		enroll[p, c, st]
	}
}
